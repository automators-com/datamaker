"use client";
import { Dialog, Transition } from "@headlessui/react";
import { Fragment, useState } from "react";
import DropDown from "../../../components/dropdown";
import { DataSources } from "../../../utilities/constants";
import type { dropDownList } from "../../../utilities/types";
import UploadData from "./upload";

export default function GenerateModal({
  isOpen,
  setIsOpen,
}: {
  isOpen: boolean;
  setIsOpen: (isOpen: boolean) => void;
}) {
  const [selectedDataSource, setSelectedDataSource] = useState(DataSources[0]);

  return (
    <>
      <Transition appear show={isOpen} as={Fragment}>
        <Dialog
          as="div"
          className="relative z-10"
          onClose={() => setIsOpen(false)}
        >
          <Transition.Child
            as={Fragment}
            enter="ease-out duration-300"
            enterFrom="opacity-0"
            enterTo="opacity-100"
            leave="ease-in duration-200"
            leaveFrom="opacity-100"
            leaveTo="opacity-0"
          >
            <div className="fixed inset-0 bg-black bg-opacity-25" />
          </Transition.Child>

          <div className="fixed inset-0 overflow-y-auto">
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              <Transition.Child
                as={Fragment}
                enter="ease-out duration-300"
                enterFrom="opacity-0 scale-95"
                enterTo="opacity-100 scale-100"
                leave="ease-in duration-200"
                leaveFrom="opacity-100 scale-100"
                leaveTo="opacity-0 scale-95"
              >
                <Dialog.Panel className="transform overflow-hidden rounded-md bg-base-100 p-6 text-left align-middle shadow-xl transition-all">
                  <Dialog.Title
                    as="h3"
                    className="text-lg font-medium leading-6 text-base-content"
                  >
                    Select a data source
                  </Dialog.Title>
                  <div className="mt-2">
                    <p className="my-6 text-sm text-base-content">
                      We will use AI to automatically generate a template from
                      the data provided.
                    </p>
                  </div>
                  <div className="mt-2">
                    <DropDown
                      list={DataSources}
                      name="target"
                      setValue={(selected: dropDownList) =>
                        setSelectedDataSource(selected)
                      }
                      value={selectedDataSource}
                      label="Select data source"
                      addClass="mb-7"
                    />
                  </div>

                  {selectedDataSource.id === 1 ? (
                    <div className="mt-2">
                      <UploadData />
                    </div>
                  ) : (
                    <div className="mt-2">
                      <p className="my-6 text-sm text-base-content">
                        This feature will be coming soon.
                      </p>
                    </div>
                  )}

                  <div className="mt-4">
                    <button
                      type="button"
                      className="btn btn-secondary"
                      onClick={() => setIsOpen(false)}
                    >
                      Abort
                    </button>
                  </div>
                </Dialog.Panel>
              </Transition.Child>
            </div>
          </div>
        </Dialog>
      </Transition>
    </>
  );
}
